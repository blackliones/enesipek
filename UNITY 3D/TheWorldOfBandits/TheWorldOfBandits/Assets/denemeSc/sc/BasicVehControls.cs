...
public class BasicVehControls : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        FL = GameObject.Find("clsFLC").GetComponent<WheelCollider>();
        FR = GameObject.Find("clsFRC").GetComponent<WheelCollider>();
        RL = GameObject.Find("clsRLC").GetComponent<WheelCollider>();
        RR = GameObject.Find("clsRRC").GetComponent<WheelCollider>();
		
        COM = GameObject.Find("carmass");
		...
		for(int i =1; i<=16; ++i) 
		{
		//	CarSound.Add(GameObject.Find(string.Format("sound {0}",i)).GetComponent<AudioSource>());
			CarSound[i-1].Play();
		}
